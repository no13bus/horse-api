...
import { UserRole } from "../src/guards/roles.guard";
...
describe("HorseController (e2e)", () => {
    ...
    it("should combine multiple filters", async () => {
      ...
      expect(
        response.body.every(
          (horse) =>
            horse.breed === "Arabian" &&
            horse.healthStatus === HealthStatus.HEALTHY,
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Advanced Filtering", () => {
    const testOwner = createTestOwner("advanced_filter");

    beforeAll(async () => {
      // Clean database before advanced filtering tests
      await prisma.$transaction([
        prisma.horse.deleteMany(),
        prisma.owner.deleteMany(),
      ]);
    ...
    it("should handle age range filtering", async () => {
      ...
      expect(response.body).toHaveLength(1);
      expect(response.body[0].name).toBe("Adult Arabian");
    });
  });
});
